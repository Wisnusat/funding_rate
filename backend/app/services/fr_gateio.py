...
class Gateio:
    ...
    def fetch_funding_rate_history(page, limit, time, sort_order, keyword):
        ...
        tickers_paginated = Gateio.paginate_list(tickers, page, limit)

        with ThreadPoolExecutor() as executor:
            results = list(executor.map(lambda ticker: Gateio.fetch_funding_data(ticker, time), tickers_paginated))
